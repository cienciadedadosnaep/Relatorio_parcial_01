# Elaboração de site com estatísticas históricas da cidade de Salvador
...
```{r recossa, fig.cap='Desenho esquemático do site (re)Conhecendo Salvador.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/mapaconceitualressa.png")
```

Um processo de licitação está em andamento para a contratação de empresa para realizar as etapas (2) e (3). A produção do conteúdo (etapa 5) está em andamento através da definição das perguntas, do levantamento (*webscraping*) dos dados e estatísticas para responder a estas perguntas e da definição de gráficos que sejam visualmente atrativos e que permitam a visualização de tendências e padrões (quando existirem). No **Apêndice 12.1** são listadas perguntas trazidas pelos estudantes e levantadas pela equipe cujos dados e estatísticas já foram inferidos.
...
```{r steprecossa, fig.cap='Fluxo do processo de desenvolvimento do site (re)Conhecendo Salvador.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image108.png")
```

## Relato da contratação da empresa de design gráfico Alinhavo Jr. e entrega da identidade visual e página principal do site (re)Conhecendo Salvador 
...
```{r logorecossa, fig.cap=' Logomarca oficial do site (re)Conhecendo Salvador.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r ilustrecossa, fig.cap='Apresentação e ilustração/menu do site (re)Conhecendo Salvador', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image29.png")
```

```{r sobrerecossa, fig.cap='Seção “Sobre o Projeto”.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image7.png")
```

```{r equiperecossa, fig.cap='Seção “Equipe”.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image23.png")
```

```{r contatorecossa, fig.cap='Seção “Contato”.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r logotrecossa, out.width = "50%",fig.cap='Logomarca da versão teste.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image11.png")
```

```{r botaotrecossa,out.width = "150%", fig.cap='Botões do menu principal', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image80.png")
```

```{r voltartrecossa, out.width = "50%",fig.cap='Botão de rodapé.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image69.png")
```


```{r botaointtrecossa, out.width = "50%",fig.cap='Botões dos painéis dos subtemas educação, população e segurança, de cima para baixo, respectivamente.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r teleinicialtrecossa, out.width = "100%",fig.cap='Tela inicial/ Página Principal.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image61.png")
```


```{r painelsubtrecossa, out.width = "100%",fig.cap='Painel do submenu do tema.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image74.png")
```


```{r paineldisptrecossa, out.width = "100%",fig.cap='Painel de perguntas disparadoras.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image92.png")
```


```{r dashtrecossa, out.width = "100%",fig.cap='Painel de perguntas disparadoras.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r reunicossa, fig.cap='Exploração dos temas de Segurança Pública e de Mobilidade Urbana.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r dashm1cossa, fig.cap=' Dashboards  com dados de número de óbitos anuais de Salvador apresentados em gráficos de linha e caixa de seleção de classe.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image65.png")
```

```{r dashm2cossa, fig.cap='Destaque da caixa de seleção de faixas etárias.', echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("images/image6.png")
```

```{r dashm3cossa, fig.cap='Destaque para o comportamento de resposta do gráfico ao movimento do ponteiro do mouse.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r dashm4cossa, fig.cap='Dashboard de Crimes Violentos contra o Patrimônio, é formado por 3 abas com séries temporais dos crimes Roubo à ônibus (destacada em amarelo), Roubo de Veículo e Furto de Veículo.', echo=FALSE, message=FALSE, warning=FALSE}
...
```{r dashm5cossa, fig.cap='Dashboard de população com dados do Censo de 1991, 2000 e 2010 disponíveis como pirâmide etário e do lado direito a evolução da população de Salvador a partir do século XVI.', echo=FALSE, message=FALSE, warning=FALSE}
